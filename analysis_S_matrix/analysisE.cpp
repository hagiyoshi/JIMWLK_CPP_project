...
void Load_matrix_V(std::complex<double>* V_matrix, const double number_rapidity, const int number)
{
	...
	ifilename << "I:\\hagiyoshi\\Data\\JIMWLK\\JIMWLK_matrix\\matrix_V_num_" << number << "_" << NX << "_INITN_" << INITIAL_N << "_" << number_rapidity << ".txt";
...
void assemble_initial_Quark_position( const int number_upper)
{
	...
	for (int i = 0; i < number_upper; ++i) {
		...
		ofilename_i2 << "I:\\hagiyoshi\\Data\\JIMWLK\\JIMWLK_matrix\\CQ_position_num_" << i << "_NX_" << NX << "_INITN_" << INITIAL_N << "_" << 0 << ".txt";
	...
	output << "I:\\hagiyoshi\\Data\\JIMWLK\\JIMWLK_matrix\\CQ_position_NX_" << NX << "_INITN_" << INITIAL_N << ".txt";
...
void print(std::complex<double>* D_matrix,double* relative_distance,const double number_rapidity, const int position)
{
	...
	std::ostringstream ofilename_i2;
	ofilename_i2 << "I:\\hagiyoshi\\Data\\JIMWLK\\JIMWLK_matrix\\D_matrix_position_" << position << "_NX_" << NX << "_rapidity_" << number_rapidity << ".txt";
...
void print_1step(std::complex<double>* D_matrix, double* relative_distance, const double number_rapidity, const int position,const int number)
{
	...
	std::ostringstream ofilename_i2;
	ofilename_i2 << "I:\\hagiyoshi\\Data\\JIMWLK\\JIMWLK_matrix\\D_matrix_position_" << position << "_NX_" << NX << "_rapidity_" << number_rapidity << "_number_" << number << ".txt";
	std::ofstream ofs_res_i2(ofilename_i2.str().c_str());
...
void Derive_Wigner_distribution(std::complex<double>* V_matrix)
{
	double h = 1.0*LATTICE_SIZE / NX;

	std::vector<double> b_position, b_integrated_value;
	...
	for (int i = 0; i < NX; i++) {
		...
		for (int j = 0; j < i; j++) {
			...
			std::vector<double> temp_vector, temp_integrated;
			impact_paramb = sqrt((1.0*i - NX / 2.0 + 1.0 / 2.0)*(1.0*i - NX / 2.0 + 1.0 / 2.0)
			+ (1.0*j - NX / 2.0 + 1.0 / 2.0)*(1.0*j - NX / 2.0 + 1.0 / 2.0))*h;
			integrated = NX * i;
